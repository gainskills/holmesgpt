...
class TestServiceNowTablesConfig:
    ...
    def test_config_api_key_valid(self):
        config = ServiceNowTablesConfig(
            api_url="https://example.service-now.com", api_key="now_abc"
        )
    ...
    def test_config_both_authentication_methods_fails(self):
        with pytest.raises(
            ValueError,
            match="authentication method must be either api key or basic auth, not both",
        ):
            ServiceNowTablesConfig(
                api_url="https://example.service-now.com",
                api_key="now_abc",
    ...
    def test_config_username_without_password_fails(self):
        with pytest.raises(
            ValueError, match="password is required when username is set"
        ):
            ServiceNowTablesConfig(
                api_url="https://example.service-now.com",
                username="api-user",
    ...
    def test_config_password_without_username_fails(self):
        with pytest.raises(
            ValueError, match="username is required when password is set"
        ):
            ServiceNowTablesConfig(
                api_url="https://example.service-now.com",
                password="secret-pass",
